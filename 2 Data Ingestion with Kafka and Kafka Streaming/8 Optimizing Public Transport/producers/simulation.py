...
class TimeSimulation:
    ...
    def __init__(self, sleep_seconds=5, time_step=None, schedule=None):
        ...
        if not isinstance(self.time_step, datetime.timedelta):
            raise ValueError("time_step must be a datetime.timedelta instance")
    ...
    def run(self):
        curr_time = datetime.datetime.now(datetime.timezone.utc).replace(
        ...
        try:
            while True:
                ...
                if self.time_step is None:
                    raise ValueError("time_step must not be None during simulation run")
